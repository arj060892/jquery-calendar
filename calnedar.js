...
$(function () {
	...
	$(".data-viewer").change(function () {
		...
		startOfWeek = moment()
			.utcOffset(timeZoneDiff)
			.startOf("isoWeek");
		currentDate = moment("02/12/2019").utcOffset(timeZoneDiff);
		if (dataView == "pol") {
			viewType = dataView;
			$(".activity-routine").hide();
			$(".pattern-of-life").show();
			...
			$(".activity-routine .table-responsive").css({
				height: $(".activity-routine").height() -
					$(".activity-routine>div:first").height() -
					40 +
					"px",
				"overflow-y": "scroll"
			});
		...
		.done(function (res) {
			if (res) {
				...
				for (var key in res.ActivityTimelineDetails) {
					...
					var acLatitude = res.ActivityTimelineDetails[key]["PlaceCoordinates"] ?
						res.ActivityTimelineDetails[key]["PlaceCoordinates"].Coordinates
						.Latitude :
						0;
					var acLongitude = res.ActivityTimelineDetails[key]["PlaceCoordinates"] ?
						res.ActivityTimelineDetails[key]["PlaceCoordinates"].Coordinates
						.Longitude :
						0;
						//TODO : temp fix
					...
					var acTravelPointDtls =
						res.ActivityTimelineDetails[key]["TravelPointLocationInformation"];
					if (acTravelPointDtls) {
						...
						acFullBeginAddress = acBeginPlace ?
							"<b>" +
							acBeginPlace +
							'</b><div style="font-size: 12px;color: grey;">' +
							acBeginAddress +
							"</div>" :
							"<b>" + acBeginAddress + "</b>";
						acFullEndAddress = acEndPlace ?
							"<b>" +
							acEndPlace +
							'</b><div style="font-size: 12px;color: grey;">' +
							acEndAddress +
							...
							"<b>" + acEndAddress + "</b>";
						acBeginLat =
							acTravelPointDtls["BeginCoordinates"]["Coordinates"]["Latitude"];
						acBeginLng =
							acTravelPointDtls["BeginCoordinates"]["Coordinates"]["Longitude"];
						acEndLat =
							acTravelPointDtls["EndCoordinates"]["Coordinates"]["Latitude"];
						acEndLng =
							acTravelPointDtls["EndCoordinates"]["Coordinates"]["Longitude"];

						//TODO: temp fix from UI
							if (acEndLat == '0' || acEndLng == '0') {
								for (let index = (parseInt(key) + 1); index < res.ActivityTimelineDetails.length; index++) {
									if (res.ActivityTimelineDetails[index]["PlaceCoordinates"]) {
										acEndLat =
											res.ActivityTimelineDetails[index]["PlaceCoordinates"].Coordinates
											.Latitude;
										acEndLng =
											res.ActivityTimelineDetails[index]["PlaceCoordinates"].Coordinates
											.Longitude;
break;
					...
					var acStartTime = moment(
						res.ActivityTimelineDetails[key]["StartTime"]
					).format("hh:mm A");
					var acEndTime = moment(
						res.ActivityTimelineDetails[key]["EndTime"]
					).format("hh:mm A");
					var hrs =
						res.ActivityTimelineDetails[key]["TotalTime"].split(":")[0] + "h";
					var min =
						res.ActivityTimelineDetails[key]["TotalTime"].split(":")[1] + "m";
					var acTotalTime = hrs + " " + min;
					var onImgClickEvnt =
						acPolCat.toLowerCase() == "travel" ?
						"markRoute(" +
						acBeginLat +
						"," +
						acBeginLng +
						...
						acEndLng +
						");" :
						"setMapCenter(" + acLatitude + ", " + acLongitude + ");";

					var addStr =
						acPolCat.toLowerCase() == "travel" ?
						"<span><b>Begin Address:</b> " +
						acFullBeginAddress +
						"</span></br><span><b>End Address:</b> " +
						acFullEndAddress +
						"</span>" :
						"<span>" +
						(acPlaceName ?
							"<b><div>" +
							acPlaceName +
							'</div></b><div style="font-size: 12px;color: grey;" class="stop-address">' +
							acAddress +
							"</div>" :
							"<b><div>" + acAddress + "</div></b>") +
						"</span>";
					tdString =
						'<td><div><img src="' +
						icons[acPolCat].icon +
						'" alt="' +
						stopTypesComp[acPolCat].Name +
						'" onClick="' +
						onImgClickEvnt +
						'"/></div></td><td><span>' +
						acStartTime +
						" - " +
						acEndTime +
						'</span></br><span style="font-size:12px;color:grey;">' +
						acTotalTime +
						"</span></td><td>" +
						addStr +
						"</td><td></td><td></td>";
					var travelGroup =
						acPolCat.toLowerCase() == "travel" ? "travelGroup" : "";
					trString +=
						'<tr class="' +
						travelGroup +
						" " +
						(acLatitude.toString().replace(".", "") +
							acLongitude.toString().replace(".", "")) +
						'">' +
						tdString +
						"</tr>";
					if (
						res.ActivityTimelineDetails[key]["PlaceCoordinates"] != null &&
						(res.ActivityTimelineDetails[key]["PlaceCoordinates"].Coordinates
							.Longitude != 0 ||
							res.ActivityTimelineDetails[key]["PlaceCoordinates"].Coordinates
							.Latitude != 0)
						...
						latLongCollection.push({
							position: new google.maps.LatLng(
								res.ActivityTimelineDetails[key][
									"PlaceCoordinates"
							...
							startTime: moment
								.utc(res.ActivityTimelineDetails[key]["StartTime"])
								.format("DD MMM YYYY hh:mm a"),
							endTime: moment
								.utc(res.ActivityTimelineDetails[key]["EndTime"])
								.format("DD MMM YYYY hh:mm a"),
							colorCode: stopTypesComp[acPolCat].Color,
							isCurrentDate: false,
							duration: acTotalTime,
							lat: res.ActivityTimelineDetails[key]["PlaceCoordinates"].Coordinates
								.Latitude,
							lng: res.ActivityTimelineDetails[key]["PlaceCoordinates"].Coordinates
								.Longitude
						});
					}
				}
...
function bindPolCal() {
	...
	for (var key in stopTypesComp) {
		...
		var tempString =
			'<div class="col-xs-2 catColorBox"><i class="fa fa-fw fa-lg fa-stop" style="color:' +
			stopTypesComp[key].Color +
			'" ></i></div><div class="row catText"><div class="col-xs-10">' +
			stopTypesComp[key].Name +
			"</div>";
		tBodyElem +=
			'<tr><th scope="row">' + tempString + "</th>" + tbodyTdElem + "</tr>";
	}
		...
		if (calView == "weeks") {
			if (
				moment(
					...
					.utcOffset(timeZoneDiff)
					.format("MM/DD/YYYY")
				).isSame(startOfWeek.format("MM/DD/YYYY"))
			) {
			...
			dayWord =
				weekDaysArr[startOfWeek.day() == 0 ? 6 : startOfWeek.day() - 1][
					"abbreviation"
				];
			...
			tHeadElem +=
				...
				'">' +
				dayWord +
				'<br><span class="bolder">' +
				startOfWeek.date() +
				"</span></th>";
			$(".table tr td:nth-child(" + (i + 2) + ")")
				.removeAttr("class")
				.addClass(startOfWeek.format("MM-DD-YYYY"))
				.addClass(todayClass);
...
function mapDatatoCal(dtTemp, month, year) {
	...
	for (var innerKey in dtTemp.PolCategories) {
		...
		$(
			"." + year + " ." + month + " ." + dtFilter + " ." + dtPOLCategory + ""
		).text(dtTotalCategoryTime);
		for (var catKey in dtInnerTemp.ActivityRoutineDetails) {
			var dtCatTemp = dtInnerTemp.ActivityRoutineDetails[catKey];
			// Temp fix
			if (
				dtCatTemp.Coordinates != null &&
				(dtCatTemp.Coordinates.Coordinates.Longitude != 0 ||
					dtCatTemp.Coordinates.Coordinates.Latitude != 0)
			) {
				var pointDuration =
					dtCatTemp.TotalTime.split(":")[0] +
					"h" +
					" " +
					dtCatTemp.TotalTime.split(":")[1] +
					"m";
				latLongCollection.push({
					position: new google.maps.LatLng(
...
function setMarkers(icons, alignMap = true) {
	...
	for (var key in groupedColl) {
		...
		var multipleLatLong =
			'<div style="font-size:12px; margin-top:11px" class="' +
			featureClass +
			'"><table>';
		features.forEach(function (feature, i) {
			multipleLatLong +=
				'<tr><td style="padding-bottom: 5px;"><b>Start Time: </b></td><td style="padding-bottom: 5px;">' +
				feature.startTime +
				"</td></tr>" +
				'<tr><td style="padding-bottom: 5px;"><b>End Time: </b></td><td style="padding-bottom: 5px;">' +
				feature.endTime +
				"</td></tr>" +
				'<tr><td style="padding-bottom: 10px;"><b>Duration: </b></td><td style="padding-bottom: 10px;">' +
				feature.duration +
				"</td></tr>";
	...
	pointerCollection.forEach(function (feature) {
		...
		google.maps.event.addListener(
			...
			(function (mm, tt) {
				return function () {
					var infoContent =
						'<div class="infowindow"><div style="margin-bottom:4px;font-size:15px;"><a href="javascript:void(0)" onclick="setStreetView(' +
						feature.lat +
						"," +
						feature.lng +
						')"><b>' +
						(feature.placeName != null ? feature.placeName : feature.address) +
						'</b></a></div><div style="margin-bottom:4px;font-size:12px;">( ' +
						feature.catName +
						" )</div>" +
						(feature.placeName != null ?
							'<div style="margin-bottom:5px;font-size:12px;color:grey">' +
							feature.address +
							"</div>" :
							"") +
						"" +
						feature.startTimeEndTime +
						"</div>";
		...
		marker.addListener("click", function () {
			$(".dar tr.rowHighlight").removeClass("rowHighlight");
